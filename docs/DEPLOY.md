...
| 프론트(Vercel) | `https://psj-ai-news-hub.vercel.app` |
...
3. [ ] **Environment** 탭에서 환경 변수 5종 등록 (설계서 §4). 값은 콘솔에만 입력:
   ...
   | `CORS_ORIGIN` | 우선 `https://psj-ai-news-hub.vercel.app` (4단계에서 최종 확정) |
...
4. [ ] Deploy 실행 → 프로덕션 도메인 `https://psj-ai-news-hub.vercel.app` 접속 확인
...
1. [ ] Render → 서비스 → **Environment** → `CORS_ORIGIN` 값을 Vercel 프로덕션 도메인으로 확정
   - 값: `https://psj-ai-news-hub.vercel.app`
   - 로컬 개발도 병행하려면 콤마로 추가: `https://psj-ai-news-hub.vercel.app,http://localhost:5173`
...
4. [ ] **실데이터 렌더링**: `https://psj-ai-news-hub.vercel.app` 새로고침 → 기사 카드(제목/한국어 요약/날짜/소스 뱃지)가 실제로 표시되는지 확인.
